...
- Resolve at build time: avoid double-post — either the harness posts the model's text, or the model posts with the key. (Leaning: model posts with the key, since the data path needs to upload a file attachment anyway.)
